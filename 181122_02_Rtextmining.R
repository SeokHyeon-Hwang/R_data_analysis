...
myCorpus <- VCorpus(DirSource("dur_multi", pattern="txt"))
myCorpus
...
meta(myCorpus[[1]], tag='etc') <- "메타정보 추가"
meta(myCorpus[[1]])
...
wordFreq = sort(wordFreq, decresing = TRUE)
wordFreq

library(wordcloud)
pal <- brewer.pal(8, "Dark2")
w = names(wordFreq)
wordcloud(words = w, freq = wprdFreq,
          min.freq = 3, random.order = FALSE,
          random.color = TRUE, colors = pal)
