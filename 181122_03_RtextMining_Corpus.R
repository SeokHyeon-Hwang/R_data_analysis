...
# 03. 텍스트에서 말뭉치(Corpus)로 변환 -  
  # VectorSource(벡터 -> 데이터소스), DirSource(디렉토리 -> 데이터소스)...,
...
# 07. 단어문서행렬(Term Document Matrix, TDM)
  # TermDocumentMatrix (코퍼스를 단어문서행렬로 만들기)
  # DocumentTermMatrix (코퍼스를 문서단어행렬로 만들기)
...
frequency # 단어별 빈도
barplot(frequency[1:20], las = 2) # 막대그래프 그리기
...
wordcloud(words = w1, # 이름
          ...
          random.order = FALSE, # 가장 횟수가 많은 단어를 중심으로
          random.color = TRUE, # 여러가지 색을 랜덤하게 지정한다.
          colors = pal) # 색 지